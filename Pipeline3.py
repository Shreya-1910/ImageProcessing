...
Dataset_root = Path(r"C:\Users\Navya\Desktop\Github files\ImageProcessing\Images")
Mask_root = Path(r"9images_watershed_masks")
Mask_root.mkdir(parents=True, exist_ok=True)
Blur_root = Path(r"9images_blurred")
Blur_root.mkdir(parents=True, exist_ok=True)
Threshold_root = Path(r"9images_threshold")
Threshold_root.mkdir(parents=True, exist_ok=True)
Morphology_root = Path(r"9images_morphology")
Morphology_root.mkdir(parents=True, exist_ok=True)
...
for image_path in Dataset_root.rglob("*"):
    if image_path.is_file() and image_path.suffix.lower() in valid_exts:
        ...
        blur_out = Blur_root / class_name
        blur_out.mkdir(parents=True, exist_ok=True)
        blur_path = blur_out / f"{image_path.stem}_blurA.png"
        if cv2.imwrite(str(blur_path), blur_imgA):
            print(f"[OK] saved blurred image: {blur_path}")
        else:
            print(f"[FAIL] could not save blurred image: {blur_path}")

        class_name = image_path.parent.name
        thresh_out = Threshold_root / class_name
        thresh_out.mkdir(parents=True, exist_ok=True)
        thresh_path = thresh_out / f"{image_path.stem}_otsu.png"
        if cv2.imwrite(str(thresh_path), otsu_thresh):
            print(f"[OK] saved thresholded image: {thresh_path}")
        else:
            print(f"[FAIL] could not save thresholded image: {thresh_path}")

        class_name = image_path.parent.name
        morph_out = Morphology_root / class_name
        morph_out.mkdir(parents=True, exist_ok=True)
        morph_path = morph_out / f"{image_path.stem}_closing.png"
        if cv2.imwrite(str(morph_path), closing):
            print(f"[OK] saved morphology image: {morph_path}")
        else:
            print(f"[FAIL] could not save morphology image: {morph_path}")
...
# pred_mask = cv2.imread('Ground Truth some/EO 2K-PBC Train (1160)_mask.png', cv2.IMREAD_GRAYSCALE)
#
